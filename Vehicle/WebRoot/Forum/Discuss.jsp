  		...
  		<table style="text-align: center;">
  			<thead>
  				<tr style="background-color: RGB(196,196,196)">
  					...
  					<th style="width: 200px">评论用户</th>
  					<th style="width: 200px">评论时间</th>
  					<th style="width: 150px">&nbsp;操&nbsp;&nbsp;作</th>
  				</tr>
  			</thead>
  			<tbody>
				<c:forEach items="${Discuss}" var="discuss" varStatus="D">
					<tr>
						<td>${D.index+1}</td>
						<td>${discuss.title}</td>
						<td>${discuss.text}</td>
						<td>${discuss.name}</td>
						<td>${discuss.time}</td>
						<td><a href="javascript:void(0)" onclick="reply('${discuss.title}','${discuss.name}')">评论回复</a></td>
					</tr>
				</c:forEach>
  			</tbody>
  		</table>
